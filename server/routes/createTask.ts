...
export default async function createTask(url: string, data: Object, db: TaskRepository): Promise<ApiResponse> {
    ...
    const task = makeTask(data.id, data.description);
    return await db.createNewTask(url, task);
}